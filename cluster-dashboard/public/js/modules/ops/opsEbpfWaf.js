...
function renderEbpfWaf(data) {
  ...
  let html = `
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:12px; margin-bottom:20px;">
      <div class="glass-card" style="padding:16px; border-left:4px solid #10B981;">
        <div style="font-size:0.75rem; color:var(--text-dim); text-transform:uppercase; font-weight:700;">İncelenen HTTP/gRPC İstek</div>
      ...
      <div class="glass-card" style="padding:16px; border-left:4px solid #EF4444;">
        <div style="font-size:0.75rem; color:var(--text-dim); text-transform:uppercase; font-weight:700;">Engellenen Çekirdek (eBPF) Saldırı</div>
      ...
      <div class="glass-card" style="padding:16px; border-left:4px solid #3B82F6;">
        <div style="font-size:0.75rem; color:var(--text-dim); text-transform:uppercase; font-weight:700;">Ortalama Paket Düşürme Süresi</div>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:20px;">
      <div class="glass-card" style="padding:18px;">
        <div style="font-size:0.95rem; font-weight:700; color:#FFFFFF; margin-bottom:12px;">🌍 Coğrafi Tehdit Kaynakları & Geo-Blocking</div>
    ...
    html += `
      <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.25); padding:8px 12px; border-radius:6px; font-size:0.82rem;">
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <span style="color:var(--text-dim); font-size:0.75rem;">• ${c.attacks.toLocaleString()} saldırı</span>
        </div>
        <button class="btn btn-sm ${c.blocked ? 'btn-danger' : 'btn-secondary'}" 
          ...
          ${c.blocked ? '🚫 BLOKLU' : 'İzin Veriliyor'}
        </button>
      </div>
    `;
  });
  ...
  html += `
      ...
      <div class="glass-card" style="padding:18px;">
        <div style="font-size:0.95rem; font-weight:700; color:#FFFFFF; margin-bottom:12px;">⚔️ Engellenen L7 Saldırı Vektörleri</div>
  ...
  data.attackTypes.forEach(att => {
    ...
    html += `
      <div style="background:rgba(0,0,0,0.25); padding:10px 14px; border-radius:6px; border-left:3px solid ${sevColor};">
        ...
        <div style="font-size:0.72rem; color:var(--text-muted);">Cilium eBPF TC (Traffic Control) kernel seviyesinde drop edildi.</div>
      </div>
    `;
  });
